...
let beforeCandleStickInfo: any = null; // 上一个订阅的k线数据
let beforeMarketInfo: any = null; //上一个订阅的Market
let beforeOrderSell: any = null; //上一个订阅的orderSell
let beforeOrderBuy: any = null; //上一个订阅的orderBuy
let beforeTradeHistoryAll: any = null; //上一个订阅的市场tradeHistory
let beforeTradeHistory: any = null; //上一个订阅的tradeHistory
let beforeOpenOrders: any = null; //上一个订阅的openOrders
let beforeOrderHistory: any = null; //上一个订阅的ordersHistory
let beforePortfolio: any = null; //上一个订阅的portfolio
const clientId = common.getCookie('clientId') || common.getLS("clientId");

const subscribeCandleStick = () => {
    // 取消上一次的k线订阅
    if (beforeCandleStickInfo) webExchangeWS.deleteTopicSubscribe(beforeCandleStickInfo);
    // 订阅新的k线
    let info = {
        ...
        emitCB: (receiveData: any, totalReceive: any) => {
...
const subscribeMarketData = () => {
    // 取消上一次的Market订阅
    if (beforeMarketInfo) webExchangeWS.deleteTopicSubscribe(beforeMarketInfo);
    let info = {
...
const subscribeOrderSell = () => {
    // 取消上一次的orderSell订阅
    if (beforeOrderSell) webExchangeWS.deleteTopicSubscribe(beforeOrderSell);
    let info = {
...
const subscribeOrderBuy = () => {
    // 取消上一次的orderBuy订阅
    if (beforeOrderBuy) webExchangeWS.deleteTopicSubscribe(beforeOrderBuy);
    let info = {
...
const subscribeTradeHistoryAll = () => {
    // 取消上一次的orderBuy订阅
    if (beforeTradeHistoryAll) webExchangeWS.deleteTopicSubscribe(beforeTradeHistoryAll);
    let info = {
...
const subscribeTradeHistory = () => {
    ...
    if (beforeTradeHistory) webExchangeWS.deleteTopicSubscribe(beforeTradeHistory);
    let info = {
        topicName: topics.TRADE_RTN,
        params: {clientId: clientId}, // 测试
        emitCB: (receiveData: any, totalReceive: any) => {
...
const subscribeOpenOrders = () => {
    // 取消上一次的OpenOrder订阅
    if (beforeOpenOrders) webExchangeWS.deleteTopicSubscribe(beforeOpenOrders);
    let info = {
...
const subscribeOrderHistory = () => {
    ...
    if (beforeOrderHistory) webExchangeWS.deleteTopicSubscribe(beforeOrderHistory);
    let info = {
...
const subscribePortfolio = () => {
    // 取消上一次的portfolio订阅
    if (beforePortfolio) webExchangeWS.deleteTopicSubscribe(beforePortfolio);
    let info = {
