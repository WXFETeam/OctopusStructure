...
export default class CustomerInfo extends React.Component {
            ...
            switch (item.type) {
                ...
                case 'select':
                    dom = React.createElement(Form.Item, { key: index, className: "formItem " + item.float, name: item.key, rules: [{
                        ...
                        React.createElement(Select, { placeholder: item.placeholder, defaultValue: item.selectDefault }, item.options.map((item2, index2) => React.createElement(Option, { key: index2, value: item2.value }, item2.text))));
                    break;
                case 'upload':
    ...
    render() {
        ...
        return (React.createElement(WrapperCustomerInfoCmp, null,
            React.createElement("div", { className: "routeName" }, user === 'institution' ? '实际控制人信息' : '客户信息'),
            React.createElement(Form, { ref: (e) => this.formRef = e, className: "customerForm", initialValues: initialValues }, formContent.map((item, index) => this.getDom(item, index, initialValues))),
            React.createElement("div", { className: "customerBtn" },
