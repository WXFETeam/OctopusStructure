...
var { depthBgColor, depthTextColor, depthBuyColor, depthSellColor, depthBuyBorderColor, depthSellBorderColor, depthBorderColor, depthTooltipBgColor, depthTooltipTitColor, depthTooltipTextColor, depthTooltipBuyColor, depthTooltipSellColor } = getMulThemeProps.styleProps(chartStyleCmpList, chartCmpNS);
var buyList = [];
var sellList = [];
var current = 0; // 当前hover状态的dataIndex
var depthChart = null;
let Depth = class Depth extends React.Component {
    constructor(props) {
        super(props);
    }
    componentDidMount() {
    ...
    drawDOM() {
        ...
        // 买卖单各分成几段渲染
        let xData = []; // x轴数组
        let maxBuy, minBuy, maxSell, minSell;
        ...
        let yData1 = []; // yData1是指买的累计volume
        let yData2 = []; // yData2是指卖的累计volume
        ...
        let borderData1 = yData1.map((item) => {
            if (item == '' || item == 0) {
                return '';
            }
            else {
        ...
        let borderData2 = yData2.map((item) => {
            if (item == '' || item == 0) {
                return '';
            }
            else {
        ...
        borderData2 = borderData1.map((item) => {
            return '';
        }).concat(borderData2);
        let that = this;
                    ...
                    axisLabel: {
                        ...
                        formatter: function (params) {
                            ...
                            if (result == 0)
                                return 0;
                            if (result < 10)
                                return common.floorNumber(result, 3);
                            if (result < 1000)
                                return parseInt(params);
                            if (result < Math.pow(10, 6))
                                return `${Number(params / 1000).toFixed(1)}k`;
                            if (result < Math.pow(10, 9))
                                return `${Number(params / Math.pow(10, 6)).toFixed(2)}m`;
                            return result;
                        },
                        textStyle: {
                            color: depthTextColor,
        ...
        var echartsElement = document.getElementById('marketDepth');
    ...
    render() {
        return (React.createElement(WrapperDepthCmp, null,
            React.createElement("div", { id: "marketDepth", style: { width: '100%', height: '100%' } })));
    }
};
Depth = __decorate([
